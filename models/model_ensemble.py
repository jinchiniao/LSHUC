...
class ResNet(nn.Module):
    ...
    def forward(self, x,denoise):
        ...
        x = self.layer3(x)
        x = self.layer4(x)
        x = x*denoise.repeat(x.shape[0]//denoise.shape[0], 1,1,1)
        x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        x = self.bn(x)
        return x         
...
class LabelSmoothingLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        return loss
class enhanceActivation(nn.Module):
    ...
    def forward(self, x):
        x=x.float()
        out = torch.where(torch.abs(x)<0.05,40*torch.abs(x)**2+0.9, 20*torch.abs(x))
        out = out.type_as(x)
        return out
...
class MyDeconvNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
class MyDeconvNet_d(nn.Module):
    def __init__(self):
        super(MyDeconvNet_d, self).__init__()
        
        self.deconv1 = nn.Sequential(
            nn.ConvTranspose2d(2048, 1024, kernel_size=4, stride=2, padding=0),
            nn.BatchNorm2d(1024),
            nn.ReLU()
        )
        self.deconv2 = nn.Sequential(
        ...
        self.downsample=partial(F.interpolate,size=(3, 3), mode='bilinear', align_corners=False)
        self.relu = nn.LeakyReLU(inplace=True)
...
class Ensemble_model(pl.LightningModule):
    def __init__(self, dropout_p=0.5, classes=500):
        ...
        self.gate_enhance = MyDeconvNet()
        self.gate_denoise = MyDeconvNet_d()
    ...
    def gate_forward(self, x):
        ...
        gate_denoise_v=self.gate_denoise(sd1.transpose(0,1).flatten(1,2)[:,:,None,None])
        return sd1.transpose(0,1),gate_enhance_v,gate_denoise_v
    ...
    def forward(self, x):
        ...
        x = self.resnet18(x,gate_denoise_v)
        # (B,T, 512)->(B,T, 512)
        x = x.view(B, T, -1)
        x, h = self.gru1(x)
        ...
        return x,sd1.transpose(0,1),gate_enhance_v,gate_denoise_v

    def configure_optimizers(self):
        params_to_update=[]
        for name, param in self.named_parameters():
            if 'gate_enhance' not in name and 'id_gru' not in name and 'id_net' not in name :
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        y_pred1,sd1,gatef1,gate1= self(x1)
        sd2,gatef2,gate2=self.gate_forward(x2)
        sd3,gatef3,gate3=self.gate_forward(x3)
        ...
        crit_ctr2 = nn.TripletMarginLoss(margin=0.45)
        ...
        loss_gate = crit_ctr2(gate1,gate2,gate3)
