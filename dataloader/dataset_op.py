...
import editdistance
import copy
from dataloader.dataset_pl import Mydataset as Mydataset_origin
...
class Mydataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #feat 2 (same)
        if len(self.id_cata[str(self.id[index])])!=1:
            lst=copy.deepcopy(self.id_cata[str(self.id[index])])
            lst.remove(index)
            index2=random.choice(self.id_cata[str(self.id[index])])
        else:
            index2=index
        video_encode = []
        dat=torch.load(self.data[index2])
        try:
            video_encode = dat.get('video')
            if len(video_encode) == 0:
                print(self.data[index2], "error")
        except:
            print(self.data[index2], "error")
        frames = []
        for i in range(len(video_encode)):
            ...
            frames.append(image)
            last_frame=image
        feat2 = np.stack(frames).transpose(3, 0, 1, 2) / 255.0
        if self.cvtransforms:
            feat2 = self.cvtransforms(feat2)
        label_idx2 = dat.get('label')
        id_idx2 = self.id[index2]
        #feat 3(diff)
        video_encode = []
        while True:
            index3=random.choice(range(len(self.data)))
            if self.id[index3]!=self.id[index]:
                break
        dat=torch.load(self.data[index3])
        try:
            video_encode = dat.get('video')
            if len(video_encode) == 0:
                print(self.data[index3], "error")
        except:
            print(self.data[index3], "error")
        frames = []
        for i in range(len(video_encode)):
            ...
            frames.append(image)
            last_frame=image
        feat3 = np.stack(frames).transpose(3, 0, 1, 2) / 255.0
        if self.cvtransforms:
            feat3 = self.cvtransforms(feat3)
        label_idx3 = dat.get('label')
        id_idx3 = self.id[index3]
        return torch.FloatTensor(np.ascontiguousarray(feat)), label_idx, int(id_idx),torch.FloatTensor(np.ascontiguousarray(feat2)), label_idx2, int(id_idx2),torch.FloatTensor(np.ascontiguousarray(feat3)), label_idx3, int(id_idx3)
    ...
    def __load_data__(self):
        ...
        for (i, sample) in enumerate(samples):
            ...
            if label in self.id_cata.keys():
                self.id_cata[label].append(i)
            else:
                self.id_cata[label]=[i]
...
class XLWBDataModule(pl.LightningDataModule):
    ...
    def val_dataloader(self):
        val_split = Mydataset_origin(
            config.path, 'val',  cvtransforms=self.val_transforms, channel=self.channel,insert=self.insert)
...
class XLWBDataModule_ctr(pl.LightningDataModule):
    def __init__(self, train_transforms, val_transforms, test_transforms,channel='GREY',insert=False):
        super().__init__()
        self.train_transforms = train_transforms
        self.val_transforms = val_transforms
...
if __name__ == "__main__":
    ...
    test_split = Mydataset(
            config.path, 'test',  cvtransforms=None,channel='GREY',insert=False)
